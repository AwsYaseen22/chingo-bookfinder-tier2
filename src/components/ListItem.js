    ...
    return (
        <article className="rounded-xl bg-white shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300 p-3">
            <div className=" relative flex justify-center overflow-hidden p-3">
                <img src={movie.image} alt={movie.title} className="object-cover h-64 w-48 rounded-xl" />
            </div>

            <div className="mt-1 p-2">
                <h2 className="text-slate-700">{movie.title}</h2>
                <p className="mt-1 text-sm text-slate-400">By: {movie.author}</p>
                <p className="mt-1 text-sm text-slate-400">Publisher: {movie.publisher}</p>
